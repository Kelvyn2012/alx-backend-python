...
class UnreadMessagesManager(models.Manager):
    ...
    def for_user(self, user):
        return (
            super()
            .get_queryset()
            .filter(receiver=user, read=False)  # only unread messages for user
            .only("id", "sender", "content", "created_at")  # ✅ optimization
        )
...
class Message(models.Model):
    ...
    edited = models.BooleanField(default=False)  # ✅ flag for edited messages
    edited_by = models.ForeignKey(  # ✅ who last edited
        settings.AUTH_USER_MODEL,
        null=True,
        blank=True,
